...
def CheckURI(uri, timeout=5):
    ...
    (i.e. times out).
    '''
    # 1. Write the CheckURI function
    #    a) Takes a URI and timeout as parameters
    #    b) Returns True if a request to that URI returns a 200 OK, and False otherwise.
    try:
...
class Shortener(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if name:
            if name in memory:
                # 2. Send a 303 redirect to a known name - memory[name]
